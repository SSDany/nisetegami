shared_examples_for "multipart template" do
  ...
  context "with liquid" do
    before do
      @template.handler = "liquid"
      @template.subject = "The quick brown {{ fox }} jumps over the lazy {{ dog }}."
      @template.body_text = "The quick brown {{ fox }} jumps over the lazy {{ dog }}."
      @template.body_html = "The quick brown {{ fox }} jumps over the lazy {{ dog }}."
      @template.save!
    end

    specify { @template.reload.handler.should == 'liquid' }
  ...
  context "with erb" do
    ...
    specify { @template.reload.handler.should == 'erb' }
    specify { message.perform_deliveries.should == true }
    specify { message.deliver; Nisetegami::TestMailer.deliveries.size.should == 1 }
